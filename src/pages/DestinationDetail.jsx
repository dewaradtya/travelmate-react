...
import { FaCalendar, FaChevronRight, FaTicketAlt, FaUsers, FaPlane, FaClock, FaMapMarkerAlt, FaInfoCircle } from "react-icons/fa";
import { useParams } from "react-router-dom";

const DestinationDetail = () => {
  const { name } = useParams();
  const imageUrl = `/${name.toLowerCase()}.jpeg`;
  const fallbackImageUrl = "/default.jpg";
  ...
  const destinationInfo = {
    ...
    description: `Discover the enchanting beauty of ${name}, a destination that seamlessly blends natural wonders with rich cultural heritage. From breathtaking landscapes to vibrant local traditions, ${name} offers an unforgettable experience for every traveler.`,
    highlights: [
  ...
  const handleBooking = () => {
    alert(`You have booked ${ticketQuantity} ${ticketOption} tickets for ${name}, departing from ${departureCity}!`);
  };
  ...
  return (
    <div className="min-h-screen p-2">
      <div 
        ...
        style={{ backgroundImage: `url(${imageUrl}), url(${fallbackImageUrl})` }}
      >
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <div className="lg:w-2/3">
            <div className="bg-white border rounded-xl shadow-lg p-6 mb-8">
              <h2 className="text-3xl font-bold mb-4 text-gray-800">Trip Details</h2>
              <div className="space-y-4">
                ...
                <ul className="list-disc list-inside ml-8 space-y-2">
                  {destinationInfo.includesAccommodation && <li className="text-gray-700">Accommodation</li>}
                  {destinationInfo.includesTransportation && <li className="text-gray-700">Transportation</li>}
                  {destinationInfo.includesMeals && <li className="text-gray-700">{destinationInfo.includesMeals}</li>}
                </ul>
              </div>
            </div>
          </div>
          ...
          <div className="lg:w-1/3">
            <div className="bg-white border rounded-xl shadow-lg p-6">
              ...
              <div className="space-y-6">
                <div>
                  <label className="block mb-2 text-sm font-semibold text-gray-700">Departure City</label>
                  <div className="relative">
                    ...
                    <FaPlane className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400" size={20} />
                  </div>
                </div>

                <div>
                  <label className="block mb-2 text-sm font-semibold text-gray-700">Ticket Type</label>
                  <div className="relative">
                    <select
                      value={ticketOption}
                    ...
                    <FaTicketAlt className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400" size={20} />
                  </div>
                </div>

                <div>
                  <label className="block mb-2 text-sm font-semibold text-gray-700">Number of Travelers</label>
                  <div className="relative">
                    <input
                      type="number"
